...
int parse_args(int argc, char** argv, filesend_config_t* cf) {
    ...
    // envs
    cf->dec_key_path = getenv(PR_KEY_ENV);
        ...
        for (int i = 5; i < argc; ++i) {
            ...
            if (strcmp(arg, "--encrypt") == 0) {
                ...
                if (strcmp(mode, "symmetric") == 0) {
                    ...
                    cf->key_path = getenv(SYM_KEY_ENV);
                } else if (strcmp(mode, "asymmetric") == 0) {
                    cf->key_path = getenv(PUB_KEY_ENV);
                } else {
                fprintf(
                    stderr, 
...
int main(int argc, char** argv) {
        ...
        if (cf.use_ws) {
            if (cf.flags & ENC_FLAG_ENABLED) {
                if (cf.flags & ENC_FLAG_SYMMETRIC) {
                    if (!cf.key_path) cf.key_path = DEFAULT_SYM_KEY_PATH;
                } else {
                    if (!cf.key_path) cf.key_path = DEFAULT_PUB_KEY_PATH;
                } 
            }
    ...
    if (cf.flags & ENC_FLAG_SYMMETRIC) {
        ...
        if (load_or_create_symmetric_key(cf.key_path, sctx.key, sizeof(sctx.key)) != 0) {
            fprintf(
                stderr,
    ...
    // Asymmetric
        ...
        if (!cf.key_path) cf.key_path          = DEFAULT_PUB_KEY_PATH;
        if (!cf.dec_key_path)  cf.dec_key_path = DEFAULT_PR_KEY_PATH;

        if (!actx.is_decrypt) {
            if (load_or_create_asymmetric_key_pair(
                cf.key_path,
                cf.dec_key_path, // empty
                actx.pub_key,
            ...
            if (load_key(cf.key_path, actx.pub_key, sizeof(actx.pub_key)) != 0 ||
                load_key(cf.dec_key_path, actx.pr_key, sizeof(actx.pr_key)) != 0) {
                fprintf(
                    stderr,
